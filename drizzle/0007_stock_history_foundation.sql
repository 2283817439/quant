...
CREATE TABLE `stock_daily_bars` (
	...
	CONSTRAINT `stock_daily_bars_security_date_adj_unique` UNIQUE(`securityId`,`tradeDate`,`adjustmentType`)
);
--> statement-breakpoint
ALTER TABLE `stock_daily_bars` ADD CONSTRAINT `stock_daily_bars_securityId_securities_id_fk` FOREIGN KEY (`securityId`) REFERENCES `securities`(`id`) ON DELETE cascade ON UPDATE no action;
--> statement-breakpoint
CREATE INDEX `securities_exchange_idx` ON `securities` (`exchange`);
--> statement-breakpoint
CREATE INDEX `securities_assetType_idx` ON `securities` (`assetType`);
--> statement-breakpoint
CREATE INDEX `stock_daily_bars_security_date_idx` ON `stock_daily_bars` (`securityId`,`tradeDate`);
--> statement-breakpoint
